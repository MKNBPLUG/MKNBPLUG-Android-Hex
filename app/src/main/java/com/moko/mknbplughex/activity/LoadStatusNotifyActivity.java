...
public class LoadStatusNotifyActivity extends BaseActivity {
    ...
    public void onMQTTMessageArrivedEvent(MQTTMessageArrivedEvent event) {
        ...
        if (cmd == MQTTConstants.MSG_ID_LOAD_NOTIFY_ENABLE && flag == 0) {
            ...
            if (dataLength != 2)
                return;
            cbLoadStartNotify.setChecked(data[0] == 1);
            cbLoadStopNotify.setChecked(data[1] == 1);
        }
...
//    }
    ...
    private void setLoadStatusNotify() {
        ...
        byte[] message = MQTTMessageAssembler.assembleWriteLoadStatusNotify(mMokoDevice.deviceId, cbLoadStartNotify.isChecked() ? 1 : 0,cbLoadStopNotify.isChecked() ? 1 : 0);
        try {
            MQTTSupport.getInstance().publish(appTopic, message, appMqttConfig.qos);
        } catch (MqttException e) {
            e.printStackTrace();
