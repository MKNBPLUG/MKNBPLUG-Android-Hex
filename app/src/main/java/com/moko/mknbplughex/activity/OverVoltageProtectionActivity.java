...
public class OverVoltageProtectionActivity extends BaseActivity {
    ...
    @BindView(R2.id.cb_overvoltage_protection)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_overvoltage_protection);
    ...
    public void onMQTTMessageArrivedEvent(MQTTMessageArrivedEvent event) {
        ...
        if (cmd == MQTTConstants.MSG_ID_OVER_VOLTAGE_PROTECTION && flag == 0) {
            ...
            if (dataLength != 4)
                return;
            cbOvervoltageProtection.setChecked(data[0] == 1);
            etVoltageThreshold.setText(String.valueOf(MokoUtils.toInt(Arrays.copyOfRange(data, 1, 3))));
            etTimeThreshold.setText(String.valueOf(data[3] & 0xFF));
        }
        if (cmd == MQTTConstants.MSG_ID_OVER_VOLTAGE_PROTECTION && flag == 1) {
            if (mHandler.hasMessages(0)) {
                dismissLoadingProgressDialog();
                mHandler.removeMessages(0);
            }
...
//    }
    ...
    private void getOverVoltageProtection() {
        ...
        byte[] message = MQTTMessageAssembler.assembleReadOverVoltageProtection(mMokoDevice.deviceId);
        try {
    ...
    private void setOverloadProtection(int voltageThreshold, int timeThreshold) {
        ...
        byte[] message = MQTTMessageAssembler.assembleWriteOverVoltageProtection(mMokoDevice.deviceId, cbOvervoltageProtection.isChecked() ? 1 : 0, voltageThreshold, timeThreshold);
        try {
            MQTTSupport.getInstance().publish(appTopic, message, appMqttConfig.qos);
        } catch (MqttException e) {
            e.printStackTrace();
